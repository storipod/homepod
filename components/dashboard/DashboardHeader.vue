<template>
  <main>
    <header class="space-y-3 bg-white px-6 py-3">
      <section class="md:flex space-y-6 md:space-y-0 items-center border-b hidden">
        <div class="xl:w-2/12">
        ...
        <div class="flex w-full flex-grow justify-end">
          <div class="flex items-center gap-x-10 pb-3">
            <div class="relative w-full">
              <input type="text" v-model="form.search" placeholder="Search keyword"
                class="w-full rounded-md bg-gray-100 px-3 py-2.5 pl-32 text-sm outline-none" />
              <img src="@/assets/icons/dashboard/search.svg" alt="" class="absolute left-2 top-2" />
            </div>
            <div class="flex justify-between items-center gap-x-4">
              <div>
                <img src="@/assets/icons/dashboard/moon.svg" alt="" class="cursor-pointer h-8 w-8" />
                <!-- <img
                  v-if="isDarkMode"
              ...
              <div>
                <img src="@/assets/icons/dashboard/bell.svg" alt="" class="cursor-pointer h-8 w-8" />
              </div>
            </div>
          </div>
        </div>
      ...
      <section class="flex items-center justify-between border-b pb-3 md:hidden">
        <div class="md:w-2/12">
          <h6 class="font-medium"> Welcome {{ username ?? 'N/A' }}! 😊</h6>
        </div>
        <div class="flex justify-between items-center gap-x-4">
          <div>
            <img src="@/assets/icons/dashboard/moon.svg" alt="" class="cursor-pointer h-6 w-6" />
            <!-- <img
              v-if="isDarkMode"
              src="@/assets/icons/dashboard/light-moon.svg"
              alt=""
        ...
        <main class="w-full p-6 relative flex flex-col h-screen">
          <div class="flex-grow relative">
            ...
            <div v-for="({ header, children }, idx) in sidebarItems" :key="idx">
              <h6 :class="[header ? 'border-b-[0.2px] border-gray-300' : '']" class="py-3 pb-3 text-[#B3BBCA]">
                {{ header }}
              </h6>
              <div class="w-full space-y-1 rounded">
                <nuxt-link v-for="itm in children" @click.native="toggleSidebar" :key="itm.title"
