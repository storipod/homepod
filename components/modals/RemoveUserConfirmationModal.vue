<template>
  ...
  >
    <section class="bg-white p-6 space-y-6 max-w-screen-lg rounded-md">
      <div>
        <img src="@/assets/icons/danger.svg" alt="warning" />
      </div>
      <div>
        <h1 class="font-semibold text-lg">Remove user</h1>
      </div>
      <div>
      ...
      <div class="flex justify-end items-end gap-x-3 w-full pt-6">
        <button
          @click="$emit('close')"
          class="text-black font-medium w-full border-gray-400 border px-3 py-2.5 rounded-lg"
        >
          Cancel
        </button>
        <button
          @click="$emit('proceed')"
          class="bg-[#D92D20] w-full text-white font-medium px-6 py-2.5 rounded-lg"
        >
          Remove
        </button>
      </div>
